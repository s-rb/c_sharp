...
namespace Homework_Theme_03
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string infoMsg = "Требуется вводить числа 1, 2, 3, 4";
            Console.WriteLine(infoMsg);

            int userTry;

            while (gameNumber != 0) {
            	userTry = GetValidUserTry(user1Name, infoMsg);
            	gameNumber -= userTry;
            	if (gameNumber == 0) {
            	...
            	userTry = GetValidUserTry(user2Name, infoMsg);
				gameNumber -= userTry;
            	if (gameNumber == 0) {
        ...
        static int GetValidUserTry(string userName, string infoMsg) {
        	Console.WriteLine($"{userName} введите число");
        	int userTry = 0;
        	bool isValid = isUserTryValid(userTry);
        	while (!isValid) {
            	userTry = int.Parse(Console.ReadLine());
            	isValid = isUserTryValid(userTry);
        ...
        static bool isUserTryValid(int userTry) {
        	return userTry >= 1 && userTry <= 4;
        }
    }
}
